//! Auto-calibration: benchmark CPU vs XPU to find the FLOP threshold
...
pub fn calibrate(f32_ops: &F32Ops, bufs: &BufferCache) -> usize {
    ...
    for &(m, n, k) in test_cases {
        ...
        // Warm XPU buffer cache
        let _ = f32_ops.dispatch_transb(bufs, a_slice, b_slice, m, n, k);

        let cpu_us = bench_median(5, || {
            let _ = a.dot(&b.t());
        });
        let xpu_us = bench_median(5, || {
            let _ = f32_ops.dispatch_transb(bufs, a_slice, b_slice, m, n, k);
        });
